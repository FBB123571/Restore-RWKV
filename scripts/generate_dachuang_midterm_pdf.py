...
PROJECT_META = {
    ...
    "abstract": (
        "本项目面向边缘端轻量化去雾需求，以 Restore-RWKV（Vision-RWKV）为基线，"
        "围绕频域混合、双域高频、深度门控、空间精炼四条方向开展可插拔消融。"
        "中期已完成基线复现、插件框架、模板—全量训练、主观对比图与 500 张 hold-out 评测。"
        "V1 基线 PSNR 20.66 dB、SSIM 0.827；后续将推进难例优化与边缘端部署验证。"
    ),
}

# 大创管理系统填报正文（与 docs/MIDTERM_FORM_COPY.md 一致）
FORM_SECTIONS = [
    (
        "已取得成果",
        (
            "1. 完成 Restore-RWKV 轻量化去雾基线（Vision-RWKV V1）复现与训练，获得稳定可用权重（epoch_50），"
            "在 500 张 hold-out 测试集上 PSNR 达 20.66 dB、SSIM 达 0.827，满足边缘端轻量化部署的初步指标要求。\n\n"
            "2. 围绕 2025 年以来频域增强、双域融合、深度先验、组合损失等方向完成文献调研与方案拆解，"
            "形成 A/B/C/D 四条可插拔改进路线及出处说明，为后续论文撰写与结题答辩提供理论依据。\n\n"
            "3. 搭建「冻结 V1 骨干 + 轻量插件」消融实验框架，在不改动原模型的前提下完成四方向插件设计与"
            "模板试训—全量训练流程，输出六列主观对比图及客观指标 CSV。\n\n"
            "4. 完成工程化开源建设：代码、训练脚本、评测脚本、中期报告 PDF 已推送至 GitHub 公开仓库"
            "（https://github.com/FBB123571/Restore-RWKV），便于团队协作、导师检查与成果展示。\n\n"
            "5. 撰写并完成大学生创新训练计划项目中期检查报告（含技术路线流程图、实验数据表、对比图），"
            "项目组按计划推进实验与文档整理，为结题材料积累可复用素材。"
        ),
    ),
    (
        "项目进展状况小结",
        (
            "本项目面向边缘端实时去雾需求，以 Restore-RWKV 线性复杂度视觉主干为基础，中期围绕"
            "「轻量化基线构建—文献方向验证—可插拔消融—工程化开源」四条主线推进，整体进度符合任务书安排。\n\n"
            "（一）数据与基线：已完成约 1.4 万对 hazy/clear 数据整理，128×128、MSE 训练 V1 基线"
            "（参数量约 0.028M），50 epoch 收敛，主观清晰自然。\n\n"
            "（二）改进与消融：设计 A 频域混合、B 双域高频、C 深度门控、D 空间精炼四条轻量插件，"
            "引入 V1 锚定损失与有界 scale；完成模板试训与 B/C/D 全量训练，建立一体化评测流程。\n\n"
            "（三）实验结果：V1 基线 PSNR 20.66 / SSIM 0.827；A、C 与 V1 最接近；B、D 部分难例指标偏低，"
            "已列入 epoch 扫描。V2 整网重训因灰图问题终止，确认「冻结基线+插件」为主路线。\n\n"
            "（四）团队分工：刘小凡统筹实验与仓库，杨媛迪、汪静怡、郭嘉恒参与文献、训练、作图与报告；"
            "指导教师金枝定期指导。中期报告与 PDF 已上传 GitHub。"
        ),
    ),
    (
        "项目存在问题及解决方案、项目下一步计划",
        (
            "【存在问题】①插件尚未显著超越 V1，难例提升有限；②B/D 的 epoch 选择需优化；"
            "③测试以典型图为主，浓雾子集不足；④多模态引导与端侧部署仍处前期。\n\n"
            "【解决方案】①单方向调整 scale 与锚定权重，验证集早停；②B/D 扫描 ep5/10/14 选最优；"
            "③构建浓雾难例子集，联合主客观评价；④保持可插拔消融，补充部署接口与推理脚本。\n\n"
            "【下一步计划】2026.06 完成最优 checkpoint 与难例评测；07 月整理论文级对比图并探索多模态文本引导；"
            "08 月开展边缘端测速与模型压缩；09 月撰写结题报告、完成答辩准备。"
        ),
    ),
]
...
def render_pages() -> list[Image.Image]:
    ...
    p.h1("系统填报正文（可复制至大创管理系统）")
    p.text("以下三节对应「已取得成果」「项目进展状况小结」「项目存在问题及解决方案、项目下一步计划」。", 17, C_MUTED)
    for title, body in FORM_SECTIONS:
        if not p.h2(title):
            p = flush(p)
            p.h2(title)
        if not p.text(body, 17):
            p = flush(p)
            p.h2(title)
            p.text(body, 17)
    p = flush(p)
